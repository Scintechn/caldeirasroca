    ...
    startTransition(async () => {
      ...
      if (result.ok) {
        ...
        track("contact_form_submit", {
          service: String(data.get("service") ?? "outro"),
        });
        form.reset();
      } else {
        setStatus("error");
    ...
    <form noValidate onSubmit={onSubmit} className="space-y-5">
      ...
      <div>
        ...
        <select
          id="service"
          name="service"
          defaultValue={preselectedService ?? "outro"}
          className="mt-1.5 w-full rounded-xl border border-slate-300 bg-white px-4 py-3 text-base text-slate-900 shadow-sm focus:border-accent-500 focus:outline-none focus:ring-2 focus:ring-accent-500"
        >
          <option value="" disabled>
            {t.service.placeholderOption}
          </option>
          {services.map((s) => (
          ...
          <option value="outro">
            {locale === "pt" ? "Outro / não tenho a certeza" : "Other / not sure"}
          </option>
        </select>
      </div>
